...
const HostedListings = (props) => {
    ...
    correctDate = new Date(
        correctDate.getTime() - correctDate.getTimezoneOffset() * 60000
    );
    correctDate = correctDate.toISOString();
    correctDate = correctDate.slice(0, -1);
    const [availability, setAvailability] = React.useState([
        { start: correctDate, end: correctDate },
    ]);
    ...
    const createGraph = async (allListings) => {
        ...
        data.bookings.forEach((booking) => {
            if (
                ...
                allListingIds.includes(parseInt(booking.listingId, 10))
            ) {
        ...
        x.forEach((day) => {
            ...
            bookings.forEach((booking) => {
                const idIndex = allListingIds.indexOf(
    ...
    const publishListing = (listingId) => {
        ...
        apiCall(`listings/publish/${listingId}`, 'PUT', allAvailability).then(
            (_) => {
                const currentListing = [...listings];
                const getIndex = currentListing.findIndex(
                    (obj) => obj.id === listingId
                );
    ...
    const callCreateListing = (data) => {
        apiCall('listings/new', 'POST', data).then((data) => {
            setCreateClose();
            getListings();
        });
    };
        ...
        return (
            <div>
                ...
                >
                    ...
                    <CreateDialog
                        callCreateListing={(e) => callCreateListing(e)}
                    />
                </Dialog>
            </div>
        );
        ...
        <Box sx={{ width: '100%', height: '100%' }}>
            ...
            <Dialog open={open} onClose={handleClose}>
                        ...
                        >
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <DesktopDatePicker
                                    ...
                                    value={data.start}
                                    onChange={(newVal) => {
                                        handleOnChangeDateStart(index, newVal);
                                    }}
                                    renderInput={(params) => (
                                        <TextField {...params} />
                                    )}
                                ...
                                <DesktopDatePicker
                                    ...
                                    value={data.end}
                                    onChange={(newVal) => {
                                        handleOnChangeDateEnd(index, newVal);
                                    }}
                                    renderInput={(params) => (
                                        <TextField {...params} />
                                    )}
                ...
                <DialogActions>
                    ...
                    <Button onClick={() => publishListing(publishId)}>
                        Submit
                    </Button>
                </DialogActions>
            </Dialog>
